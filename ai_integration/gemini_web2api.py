...
def load_cookie() -> tuple:
    """Load cookie from environment or file. Returns (cookie_str, sapisid)."""
    cookie_env = os.environ.get("GEMINI_COOKIE") or CONFIG.get("cookie_env")
    if cookie_env:
        content = cookie_env.strip()
        if content.startswith("{"):
            try:
                data = json.loads(content)
                return data.get("cookie", ""), data.get("sapisid", "") or None
            except Exception:
                pass
...
class GeminiHandler(BaseHTTPRequestHandler):
    ...
    def _serve_static(self, path: str) -> bool:
        """Serve static files from the project root if available."""
        candidates = [
    ...
    def do_GET(self):
        try:
            ...
            if self.path == "/" and "application/json" in accept:
